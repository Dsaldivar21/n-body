...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (numParticles <= 0 || numIterations <= 0 || outputInterval <= 0) {
            std::cerr << "Invalid input: num_particles, num_iterations, and output_interval must be positive integers." << std::endl;
            return 1;
        }

        if (dt <= 0) {
            std::cerr << "Invalid input: time_step must be a positive number." << std::endl;
            return 1;
        }
